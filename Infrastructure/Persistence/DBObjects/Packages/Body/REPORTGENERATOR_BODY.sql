...
BEGIN
    ...
    SELECT NVL(MAX(CODIGOIDENTIFICATORIO), '00000000') INTO V_SCODE FROM REPORTES;
        
    V_SCODE := DECIMAL_TO_HEXA(HEXA_TO_DECIMAL(V_SCODE) + 1);
    
    /* COMPLETAMOS CON 0 A LA IZQUIERDA HASTA TENER 8 CARACTERES */

    WHILE LENGTH(TRIM(V_SCODE)) < 8 LOOP
        V_SCODE := '0' || TRIM(V_SCODE);
    END LOOP;
  ...
  BEGIN
    OPEN REPORTCURSOR FOR
        SELECT DISTINCT 
          ...
          FROM PRESTADORES P
               INNER JOIN DOCUMENTACIONES D ON D.PRESTADORID = P.ID 
               INNER JOIN AGENDAS AG ON AG.PROFESIONALID = P.ID
